...
function Verify() {
  ...
  const handleFileChange = (e) => {
    ...
    setFileName(selectedFile ? selectedFile.name : ""); // Set filename or clear if no file
  ...
  const checkIntegrity = async () => {
    if (file) {
      ...
      try {
        const response = await fetch("https://pretty-em-ssc2334-cb2b05d6.koyeb.app/verify-hash", {
          method: "POST",
          body: formData,
        });
        ...
        if (response.ok) {
          ...
          setVerifyMessage(result.verification_result); // Display verification result
          ...
          <div className="verify-upload-section">
            ...
            {fileName && (
              <div className="file-name">
                <p>Selected file: {fileName}</p> {/* Display the selected filename */}
              </div>
            )}
            ...
            {verifyMessage && (
              <div className="verify-message">
                <p>{verifyMessage}</p> {/* Display verification result */}
              </div>
            )}
          </div>
        </div>
